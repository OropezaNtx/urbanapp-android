...
function tripId(trip) { return safe(trip?.cloudTripId || trip?.localTripId || trip?.tripId || trip?.id || 'trip'); }
function tripName(trip) { return safe(trip?.routeName || `Urban trip ${tripId(trip)}`); }
function directionKey(trip) {
  const dir = String(trip?.direction || '').trim().toUpperCase();
  if (dir.includes('REGRESO')) return 'REGRESO';
  return 'IDA';
}
function directionTitle(trip) { return directionKey(trip) === 'REGRESO' ? 'Regreso' : 'Ida'; }
function trackName(trip) { return directionKey(trip); }
function garminTrackColor(trip) { return directionKey(trip) === 'REGRESO' ? 'Cyan' : 'Red'; }
...
function fileBase(trip) { return exportBaseName(trip).replace(/[^a-zA-Z0-9 _.-]+/g, '').slice(0, 120); }
function collectTrackPoints(chunks = []) {
  return buildTrackMetrics(Array.isArray(chunks) ? chunks : []).points.filter((p) => validCoord(p.lat, p.lon));
}
...
function trackDistanceMeters(points) { return Math.round(buildTrackMetrics([{ points }]).distance || 0); }
function garminTrack(trip, points, { includeStats = false, color = garminTrackColor(trip) } = {}) {
  const extensions = includeStats ? [
    '    <extensions>',
  ...
  return ['  <trk>', `    <name>${xmlEscape(trackName(trip))}</name>`, ...extensions, '    <trkseg>', points.map(garminTrackPoint).join('\n'), '    </trkseg>', '  </trk>'].join('\n');
}
...
export function buildGarminWaypointsGpx(trip, events = []) {
  ...
  return [garminHeader({ creator: 'eTrex 20', fullGarminNamespaces: true }), garminMetadata(metadataTime), ...eventPoints.map((e, idx) => garminWaypoint(e, idx, false)), '</gpx>'].join('\n');
}
export function buildGarminTrackGpx(trip, events = [], chunks = []) {
  ...
  return [garminHeader({ creator: 'eTrex 20', fullGarminNamespaces: true }), garminMetadata(metadataTime), garminTrack(trip, points, { includeStats: true, color: garminTrackColor(trip) }), '</gpx>'].join('\n');
}
export function buildMapSourceCombinedGpx(trip, events = [], chunks = []) {
  const points = collectTrackPoints(chunks);
  const eventPoints = collectEventPoints(events);
  return [garminHeader({ creator: 'MapSource 6.16.3', fullGarminNamespaces: false }), garminMetadata(nowIso(), boundsXml(events, points)), ...eventPoints.map((e, idx) => garminWaypoint(e, idx, true)), garminTrack(trip, points, { includeStats: false, color: garminTrackColor(trip) }), '</gpx>'].join('\n');
}
export function buildGpx(trip, events = [], chunks = []) { return buildMapSourceCombinedGpx(trip, events, chunks); }
...
export function buildKml(trip, events = [], chunks = []) {
  ...
  return [
    ...
    `        <Icon><href>${KML_BLUE_FLAG_ICON}</href></Icon>`,
    '      </IconStyle>',
...
export function buildGeoJson(trip, events = [], chunks = []) {
  ...
  const features = [];
  if (points.length) {
    features.push({ type: 'Feature', properties: { kind: 'track', name: trackName(trip), tripId: tripId(trip), pointCount: points.length }, geometry: { type: 'LineString', coordinates: points.map((p) => [Number(p.lon), Number(p.lat), Number(p.alt ?? 0)]) } });
  }
...
export function buildTrackCsv(trip, events = [], chunks = []) {
  ...
  const points = collectTrackPoints(chunks);
  const rows = points.map((p, idx) => ({ tripId: tripId(trip), routeName: tripName(trip), idx: idx + 1, time: fmtLocal(p.time), lat: p.lat, lon: p.lon, alt: p.alt, accuracy: p.accuracy, provider: p.provider, chunkId: p.chunkId, chunkIndex: p.chunkIndex, pointIndex: p.pointIndex }));
...
function buildZipSingleFile(filename, content) {
  ...
  writeU32(local, crc); writeU32(local, data.length); writeU32(local, data.length); writeU16(local, nameBytes.length); writeU16(local, 0); writeBytes(local, nameBytes); writeBytes(local, data);
  ...
  writeU32(central, crc); writeU32(central, data.length); writeU32(central, data.length); writeU16(central, nameBytes.length); writeU16(central, 0); writeU16(central, 0); writeU16(central, 0); writeU16(central, 0); writeU32(central, 0); writeU32(central, 0); writeBytes(central, nameBytes);
  const end = [];
...
export function downloadGarminWaypointsGpx(trip, events) { downloadText(`Waypoints_${fileBase(trip)}.gpx`, buildGarminWaypointsGpx(trip, events), 'application/gpx+xml'); }
export function downloadGarminTrackGpx(trip, events, chunks) { downloadText(`Track_${fileBase(trip)}.gpx`, buildGarminTrackGpx(trip, events, chunks), 'application/gpx+xml'); }
export function downloadMapSourceCombinedGpx(trip, events, chunks) { downloadText(`${fileBase(trip)}_Juntos.gpx`, buildMapSourceCombinedGpx(trip, events, chunks), 'application/gpx+xml'); }
export function downloadGpx(trip, events, chunks) { downloadMapSourceCombinedGpx(trip, events, chunks); }
export function downloadKml(trip, events, chunks) { downloadText(`${fileBase(trip)}.kml`, buildKml(trip, events, chunks), 'application/vnd.google-earth.kml+xml'); }
export function downloadKmz(trip, events, chunks) { downloadBlob(`${fileBase(trip)}.kmz`, buildZipSingleFile('doc.kml', buildKml(trip, events, chunks))); }
export function downloadGeoJson(trip, events, chunks) { downloadText(`${fileBase(trip)}.geojson`, buildGeoJson(trip, events, chunks), 'application/geo+json'); }
export function downloadTrackCsv(trip, events, chunks) { downloadText(`${fileBase(trip)}_track_points.csv`, buildTrackCsv(trip, events, chunks), 'text/csv'); }
export function buildExportStats(events = [], chunks = []) {
  const track = buildTrackMetrics(Array.isArray(chunks) ? chunks : []);
  const eventPoints = collectEventPoints(events);
  return { pointCount: track.totalPoints, eventPointCount: eventPoints.length, distanceKm: track.distance / 1000, durationMs: track.durationMs, avgAccuracy: track.avgAccuracy };
}
